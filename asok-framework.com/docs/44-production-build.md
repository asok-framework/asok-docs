...
> **`SECRET_KEY` and live components:** If your project embeds live (WebSocket) components in SSG pages, export your production `SECRET_KEY` **before** running `asok build` so their pre-rendered state is signed with your real key. Use the same key at run time. Building without a `SECRET_KEY` still produces valid static pages, but live components on SSG pages will not hydrate in production (a warning is printed). See [SSG & ISR](53-ssg-isr.md) → *the `SECRET_KEY` Contract* for details.
