...
#### Precise Error Localization & Line Numbers

To simplify development and debugging, Asok dynamically tracks line numbers through the template compilation process. When an error occurs during template compilation or rendering, Asok raises a `TemplateError` that includes:
- The template filename or path (if rendered via `Request.html()` or with `template_name` provided).
- The exact line number in the template file where the error occurred.
- A descriptive error message.

For example, referencing a division by zero in `page.html` yields:
```
Template 'page.html' Syntax Error: division by zero on line 14
```

#### Unclosed Delimiters Validation

Asok validates that all template expressions (`{{ ... }}`) and statement blocks (`{% ... %}`) are properly opened and closed. If a delimiter is left unclosed (e.g., `{{ name`), the compiler detects the syntax error at compile time and raises a `TemplateError` pointing to the exact line number of the unclosed delimiter:
```
Template 'home.html' Syntax Error: Unclosed template expression '{{' on line 5
```

#### Rich HTML Debug Page

When `DEBUG = True`, any uncaught `TemplateError` is intercepted by Asok's WSGI layer to serve a beautifully styled HTML debug page. This page contains:
- **Error Summary**: The template filename, line number, and error message.
- **Python Traceback**: The full traceback highlighting the exact call stack.
- **Compiled Python Code**: The raw Python generator code generated by the template engine, annotated with line numbers.
- **Context Variables**: A table showing all request parameters and variables active during the request.

In production (`DEBUG = False`), the technical details are hidden, a standard error page is returned, and the full exception is logged securely on the server.
