...
If you provide a callable, Asok automatically resolves it at request time. This allows you to share dynamic data (like database query results or request parameters) across all templates.

Asok supports three types of callable resolution:

#### 1. Request-aware factories
If the callable's first parameter is named `request` or `req`, it is executed with the current `Request` object:
```python
# wsgi.py
app.share(user=lambda request: request.user)
```

#### 2. Zero-argument callables
If the callable accepts no parameters, it is executed without arguments at request time:
```python
# wsgi.py
app.share(year=lambda: datetime.now().year)
```

#### 3. Direct method/function references
You can pass direct references to methods or functions (such as ORM classmethods). Asok will execute them dynamically for each page render:
```python
# wsgi.py
app.share(names=Name.all) # Executed per-request, database data remains fresh
```

#### Class & Module Sharing Protection

Class definitions (like ORM model types or custom classes) and Python modules are safely shared as-is. They are not auto-executed or instantiated at request time, allowing you to access static methods or class constructors directly inside your templates:
```python
# wsgi.py
import math
app.share(User=User, math=math)

# In your template, you can call class/module methods:
{{ User.get(1).name }}
{{ math.sqrt(16) }}
```

### Accessing the Application Instance (`request.app`)

To prevent circular imports (which can happen if you import `app` from `wsgi.py` in your controllers), Asok exposes the active application instance via the `request.app` property:

```python
# In any controller
def render(request):
    debug_mode = request.app.config.get("DEBUG")
```

The `request.app` property is type-hinted to the `Asok` class, giving your IDE full autocompletion/IntelliSense for all application methods and configurations.

### App-wide vs. Request-wide sharing

When sharing variables, choose the correct scope depending on where they are defined and whether they contain user-specific data:

#### 1. Application-wide sharing (`app.share()`)
Call `app.share()` at startup (usually in `wsgi.py` or configuration files). Variables shared this way are **global to the entire application** (across all threads, requests, and users):
```python
# wsgi.py
app.share(site_name="My Blog")
```
> [!WARNING]
> Do not call `request.app.share()` inside a controller to store user-specific data, as it will leak to all other users visiting the site.

#### 2. Request-wide sharing (`request.share()`)
Call `request.share()` inside a controller, middleware, or hook. Variables shared this way are **isolated and request-scoped** (only active for the current user's request lifecycle, thread-safe, and automatically cleaned up afterwards):
```python
# In a controller
def show_profile(request):
    # Safe: Only available in templates rendered during this specific request
    request.share(current_section="profile")
    return request.html("profile.html")
```

### Accessing shared variables in Python

Use `request.shared(name)` to retrieve any shared variable's resolved value for the current request context:
