...
Middleware can also be declared using `async def` for non-blocking operations (e.g., calling external APIs or querying the database with async ORM methods). Asok automatically resolves mixed sync/async chains under both ASGI and WSGI servers. For details, see [Async & Concurrency](../advanced/51-async-concurrency.md).
