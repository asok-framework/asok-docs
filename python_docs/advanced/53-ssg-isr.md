...
### Live Components on Static Pages: the `SECRET_KEY` Contract

When you embed interactive (live) components in SSG pages via the Islands Architecture (see §4), their initial state is signed at **build time** and re-validated by the running server when the component hydrates over WebSocket. For this handshake to succeed, the state must be signed with a key the production server trusts.

**Provide the same `SECRET_KEY` at build time and at run time.** Sign your build so pre-rendered component state is authenticated against your production key:

```bash
# Generate once, then reuse for every build + deploy of this release
export SECRET_KEY="$(python -c 'import secrets; print(secrets.token_hex(32))')"

asok build        # bakes SSG state signed with $SECRET_KEY
# ...deploy dist/ with the SAME SECRET_KEY in the server environment
```

> **Security:** Asok never trusts a public or placeholder key to authenticate component state in production. State is validated **only** against the server's `SECRET_KEY`.

**If you build without a `SECRET_KEY`**, the build still succeeds and static pages are served normally, but:
- `asok build` prints a warning.
- Live components embedded in SSG pages **will not hydrate** in production (their baked state cannot be authenticated against the production key). This is the secure default — it fails closed rather than trusting an unauthenticated state.

Purely static SSG pages (no live components) are unaffected and require no build-time key.

> **Tip:** A weak `SECRET_KEY` (under 32 characters) exported at build time now surfaces the production key-strength check during `asok build` instead of at first boot — fix it before deploying.
